...
const { getCommentsByBook, addComment, addReply, toggleLike } = useComments()
const { user } = useUser()

const bookComments = computed(() => getCommentsByBook(bookName.value))
...
const sendComment = () => {
  ...
  addComment(bookName.value, newComment.value, user.value?.id || 0, username)
