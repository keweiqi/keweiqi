<template>
  <view class="notes-page" :class="{ dark: isDark }">
    <view class="header">
      ...
      <view class="title">第 {{ paragraphIndex + 1 }} 段笔记</view>
...
const { getNotesByParagraph, addNote, updateNote: updateNoteInStore, deleteNote } = useNotes()

const bookNotes = computed(() => {
  return getNotesByParagraph(bookName.value, paragraphIndex.value).sort((a, b) => new Date(b.createTime) - new Date(a.createTime))
})
...
const saveNote = () => {
  ...
  addNote(bookName.value, newNote.value, paragraphIndex.value)
